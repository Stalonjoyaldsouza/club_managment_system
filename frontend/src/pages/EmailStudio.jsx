...
export default function EmailStudio() {
  ...
  const recipients = useMemo(() => [...selectedMembers.map((id) => members.find((m) => m.id === id)?.user.email).filter(Boolean), ...external.split(/[,\s]+/).filter(Boolean)], [selectedMembers, external, members]);
  const generate = useMutation({
    mutationFn: () => api.post("/email/generate", { category: form.category, subject: form.subject, notes: form.notes, tone: form.tone, eventDetails: form.notes }).then(unwrap),
  ...
  const send = useMutation({
    ...
    onSuccess: () => toast.success("Email sent and logged"),
    onError: (error) => toast.error(error.response?.data?.message || "Email failed")
  });
